    ...
    def parse(self, response):
        ...
        if checker == '분류':
            ...
            for report in reports:
                ...
                sector_item['title'] = report.xpath('./td[2]/a/text()').get()
                sector_item['source'] = report.xpath('./td[3]/text()').get()
                sector_item['file_url'] = report.xpath('./td[4]/a/@href').get()
                sector_item['date'] = report.xpath('./td[5]/text()').get()
                yield sector_item
        else:
            reports = response.xpath("//tr[td[contains(@style, 'padding-left:10')]]")
            for report in reports:
                sector_item = SectorItem()
                sector_item['title'] = report.xpath('./td[1]/a/text()').get()
                sector_item['source'] = report.xpath('./td[2]/text()').get()
                sector_item['file_url'] = report.xpath('./td[3]/a/@href').get()
                sector_item['date'] = report.xpath('./td[4]/text()').get()
                yield sector_item
